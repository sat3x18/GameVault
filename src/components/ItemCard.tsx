...
export const ItemCard: React.FC<ItemCardProps> = ({ item, onPurchase, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl overflow-hidden hover:border-blue-500/50 transition-all duration-300 hover:transform hover:scale-105 hover:shadow-2xl group">
      ...
      <div className="p-6">
        <Link to={`/item/${item.id}`} className="block">
          <h3 className="text-xl font-bold text-white mb-2 line-clamp-1 hover:text-blue-400 transition-colors">{item.title}</h3>
        </Link>
        ...
        <div className="flex items-center justify-between">
          <Link to={`/item/${item.id}`} className="block">
            <div className="text-2xl font-bold text-transparent bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text hover:from-blue-300 hover:to-purple-400 transition-all">
              ${item.price}
            </div>
          </Link>
